...
async function run() {
    try {
        ...
        // Prevent double-booking at the database level: only one active
        // (Pending/Confirmed) booking is allowed per venue per date.
        // This is enforced by MongoDB itself, so it's safe even if two
        // booking requests for the same venue+date arrive at the same time.
        ...
        app.get(
            ...
            async (req: Request, res: Response) => {
                try {
                    ...
                    res.status(500).send({ message: "Failed to fetch booked dates." });
                }
            }
        );
        ...
        app.get("/api/customer-stats/:email", async (req: Request, res: Response) => {
            try {
                const { email } = req.params;
        ...
        app.get(
            ...
            async (req: Request, res: Response) => {
                try {
                    const { venueId, email } = req.params;
                    ...
                    res.send({ canReview: !!completed && !alreadyReviewed });
                } catch (err) {
